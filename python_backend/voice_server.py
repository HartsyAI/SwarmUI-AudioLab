...
- Text-to-Speech synthesis via ChatterboxTTS (with fallbacks)
...
async def get_installation_help():
    ...
    help_info = {
        ...
        "required_dependencies": {
            "core": ["fastapi", "uvicorn", "numpy", "scipy"],
            "stt": {
                "primary": "RealtimeSTT",
...
async def health_check():
    ...
    try:
        # Check service initialization status
        stt_ready = stt_service.initialized
        tts_ready = tts_service.initialized
        
        overall_status = "healthy" if (stt_ready and tts_ready) else "degraded"
